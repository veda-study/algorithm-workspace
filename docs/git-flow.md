...
```bash
git clone [https://github.com/veda-study/algorithm-workspace.git](https://github.com/veda-study/algorithm-workspace.git)
cd veda-study
```
...
2. **Merge 수행:** 최소 1명 이상의 Approve를 받으면 PR 창 하단의 `Squash and merge` 버튼이 활성화됩니다. PR을 올린 본인이 직접 `Confirm merge`를 눌러 `main` 브랜치에 코드를 합칩니다.
